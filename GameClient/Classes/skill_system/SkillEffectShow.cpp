...
{
	//CC_SAFE_DELETE(m_attacker);
	//CC_SAFE_DELETE(m_victim);
	//CCLog("SkillEffectShow:%d",--SkillEffectShowCount);
}
...
bool SkillEffectShow::init(Monomer* attacker, Monomer* victim)
{
	...
	{
		std::string path("");
		path = TextureController::getTexturePath(path,TexturePathSkillCaster, m_skillInfo.getCasterSpecificID());
...
void SkillEffectShow::emission(float delay)
{
	...
	{
		std::string path("");
		path = TextureController::getTexturePath(path,TexturePathSkillLocus, m_skillInfo.getLocusSpecificID());
...
void SkillEffectShow::track(float delay)
{
	...
	if (ccpDistance(point, this->getDestination()) > speed)
	{
		...
		Point high = ((Monomer*)m_attacker->getParent())->getHandPointRelativeFootOffset();
...
void SkillEffectShow::hit()
{
	...
	{
		std::string path("");
		path = TextureController::getTexturePath(path,TexturePathSkillExplosion, m_skillInfo.getExplosionSpecificID());
...
void SkillEffectShow::updateTailing(float delay)
{
	...
	CCSize preferredSize = CCSizeZero;
...
const Point SkillEffectShow::getDestination()
{
...
const Point SkillEffectShow::getDeparture()
{
