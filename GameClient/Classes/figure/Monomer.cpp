...
#include "skill_system/SkillEffectShow.h"
...
void Monomer::showFigure()
{
	do
	{
		...
		bloodIn->setAnchorPoint(Point::ZERO);
...
Point Monomer::getHandPointRelativeFootOffset()
{
	...
	Point point = Point(_contentSize.width * anchorPoint.x, _contentSize.height * anchorPoint.y);//m_obContentSize
	return point;
}
...
Point Monomer::getHandPoint()
{
	...
	Point point = ccpAdd(_position, handRelativeFoot);
	return point;
}
...
Point Monomer::getHurtPointRelativeFootOffset()
{
	...
	Point point = Point(_contentSize.width * anchorPoint.x, _contentSize.height * anchorPoint.y);
	return point;
}
...
Point Monomer::getHurtPoint()
{
	...
	Point point = ccpAdd(_position, hurtRelativeFoot);
	return point;
}
...
{
	int off_x = _contentSize.width * 0.4f;
	int off_y = _contentSize.height * _anchorPoint.y * 0.5f;

	int width = _contentSize.width * 0.3f;
	int height = _contentSize.height * 0.6f;

	Rect rect;
	rect.origin = ccp(off_x, off_y);
	rect.size = Size(width, height);

	int x = _contentSize.width * _anchorPoint.x;
	int y = _contentSize.height * _anchorPoint.y;

	Point point = Point(x, y);
	point = ccpSub(_position, point);
...
void Monomer::playEffect(Node* node, const char *path)
{
	do
	{
		...
		Vector<SpriteFrame*> array(64);  //=CCArray::createWithCapacity(64);
		...
		do
		{
			...
			SpriteFrame* spriteFrame = CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(string->getCString());
...
void Monomer::followTheTracks()
{
	do
	{
		...
		point = mapSub(point, MapPoint(_position));
...
{
	MapPoint point = MapPoint(_position);
...
{
	M_INT lenghtX = mapSub(point, MapPoint(_position)).x;
	M_INT lenghtY = mapSub(point, MapPoint(_position)).z;
...
{
	MapPoint position = MapPoint(_position);
	return (bool)(ccpDistance(position.getCCPointValue(), _position) >= 5.0f);
}

Vector<FiniteTimeAction*> Monomer::actionsWithPoint(MapPoint& pStart, MapPoint& pFinish)
{
	SEL_CallFunc callfunc = NULL;

	Vector<FiniteTimeAction*> array; //= CCArray::create();
...
Vector<FiniteTimeAction*> Monomer::actionsWithMoveTo(std::deque<MapPoint>& pArray)
{
...
{
	MoveInfo relust = {0.0f, MapPoint(_position), false};
...
MoveInfo Monomer::goTo(MapPoint point)
{
	...
	do
	{
		...
		MapPoint begin = MapPoint(_position);
...
{
	float value = BgMap::getZOrder(this->getPosition());//zÖá
	this->setLocalZOrder(value);//setZOrder
	...
	do
	{
		...
		if (GAME_SCENE->getCurrBgMap()->getCurrentGridValue(point) == 2)
		{
			...
			m_nMonomer->setColor(Color3B::GRAY);
		}
		else
		{
			...
			m_nMonomer->setColor(Color3B::WHITE);
		}
	}
	while (0);
}
...
{
	this->setLocalZOrder(BgMap::getZOrderZero(GAME_SCENE->getCurrBgMap()));//setZOrder 
}