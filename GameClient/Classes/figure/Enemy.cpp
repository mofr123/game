...
#include "game_scene/FlutteringFairy.h"
...
void Enemy::onEnterTransitionDidFinish()
{
	...
	MapPoint position = MapPoint(_position);//用于设置本Node对象的锚点处在父亲节点中的位置
...
{
	if (!GAME_SCENE->getCurrBgMap()->getShowRect().containsPoint(_position))//m_obPosition
		return;
...
{
	if (GAME_SCENE->getCurrBgMap()->getHideRect().containsPoint(_position))//m_obPosition
		return;
...
void Enemy::patrol(float delay)
{
	...
	MapPoint dirt = mapSub(point, MapPoint(_position));//m_obPosition
	this->walkBy(dirt);
