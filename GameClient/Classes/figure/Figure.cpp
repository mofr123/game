...
{
	string figurePath("");
	figurePath = TextureController::getTexturePath(figurePath,type, figureNumber);
...
bool Figure::init()
{
	...
	this->setContentSize(Size(256, 256)*0.8f);//ccpMult(Size(256, 256), 0.8f)
	return true;
}
...
void Figure::setHair(unsigned int hairNumber)
{
	...
	do
	{
		...
		m_hairSprite = Sprite::create();
		m_hairSprite->setPosition(Point(128, 128) * 0.8f);
...
void Figure::setWeapon(unsigned int weaponNumber)
{
	...
	do
	{
		...
		m_weaponSprite->setPosition(Point(128, 128) * 0.8f);
...
void Figure::runActions()
{
	...
	string str("");
	string figurePath = TextureController::getTexturePath(str,m_pathType, m_figureNumber);
	...
	{
		CallFunc* callFunc = CallFunc::create(this, callfunc_selector(Figure::hurtCompleted));
	...
	{
		CallFunc* callFunc = CallFunc::create(this, callfunc_selector(Figure::deathCompleted));
		Sequence* sequence = Sequence::create(animate, callFunc, NULL);
		this->runAction(sequence);
	...
	do
	{
		...
		string str("");
		string hairPath = TextureController::getTexturePath(str,TexturePathHair, m_hairNumber);
	...
	do
	{
		...
		string str("");
		string weaponPath = TextureController::getTexturePath(str,TexturePathWeapon, m_weaponNumber);
...
{
	Vector<SpriteFrame*> array(8); //= CCArray::createWithCapacity(8);
	...
	do
	{
		...
		SpriteFrame* spriteFrame = SpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(string->getCString());
