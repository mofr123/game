...
class TextureAsync : public Ref
{
    ...
    void addSpriteFrames(Texture2D* texture)
    {
        ...
        SpriteFrameCache::getInstance()->addSpriteFramesWithFile(completePath.c_str(), texture);
    ...
    void addImageAsync()
    {
        ...
        TextureCache::getInstance()->addImageAsync(completePath.c_str(), CC_CALLBACK_1(TextureAsync::addSpriteFrames,this));//callfuncO_selector
    }
    
    void WaitForFinish(float dt)
    {
        ...
        {
            Director::getInstance()->getScheduler()->scheduleSelector(schedule_selector(TextureAsync::WaitForFinish), this, 0.1f, false);
        }
    }
...
public:
    ...
    static void addSpriteFramesAsync(string& path, Ref *target = NULL, SEL_CallFunc selector = NULL)
    {
        ...
        if (itr == textureMap.end())
        {
            if (CC_TARGET_PLATFORM == CC_PLATFORM_MAC || GAME_SCENE == NULL)
            {
                ...
                SpriteFrameCache::sharedSpriteFrameCache()->addSpriteFramesWithFile(completePath.c_str());
                if (target && selector)
                {
                    (target->*selector)();
                }
    ...
    static void subSpriteFramesAsync(string& path)
    {
        ...
        if (itr != textureMap.end())
        {
            ...
            {
                std::string completePath = path + textureSuffix;
                SpriteFrameCache::getInstance()->removeSpriteFramesFromFile(completePath.c_str());
                
                std::string pngPath = path + texturePngSuffix;
                TextureCache::getInstance()->removeTextureForKey(pngPath.c_str());
...
string& TextureController::getTextureRoute(string &path, TexturePathType type, unsigned int number)
{
    //string path;
    path.clear();
    switch (type)
...
string& TextureController::getTexturePath(string &path , TexturePathType type, unsigned int number)
{
    //string path;
    path.clear();
    switch (type)
...
void TextureController::addSpriteFrames(TexturePathType type, unsigned int number, Ref *target, SEL_CallFunc selector)
{
    do
    {
		...
		string path("");
        path = getTextureRoute(path,type, number);
