...
if ( Ref * __object__ = (Ref *)1 ) \
if ( (array) && (array)->data->num > 0 ) \
for( Ref** arr = (array)->data->arr, **end = (array)->data->arr + (array)->data->num-1; \
...
void MapPointAStar::init(const MapPoint & pos, const MapPoint & target, int _G_score)
{
	...
	double tx = target.x; 
	double tz = target.z;
    hScore = 10 * fabs(tx - position.x) + 10 * fabs(tz - position.z);
    gScore = _G_score;
}
