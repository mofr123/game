...
MapPoint::MapPoint(const Point& point)
...
Point MapPoint::getCCPointValue() const
{
    Point point = Point(x * GRID_SIZE.width, z * GRID_SIZE.height);
...
MapPoint MapPoint::operator/(M_INT a) const
{
	CCAssert(a, "Point division by 0.");
    return MapPoint((M_INT)this->x / a, (M_INT)this->z / a);
}
