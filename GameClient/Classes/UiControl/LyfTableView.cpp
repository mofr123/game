...
#define PULL_SHOW_IMAGE "ui/indicator.png"

#define UPDATE_TIME 0.3f

class LPullShowSprite: public Sprite
{

private:

	Sprite* pullShowSprite;

	Sprite* pullShowLabel;
...
public:
	...
	bool init()
	{
		...
		if (m_isTop)
		{
			pullShowSprite=Sprite::create(PULL_SHOW_IMAGE);
			this->addChild(pullShowSprite);
			pullShowSprite->setRotation(180);

			Label *label=Label::create(ON_ONE_PAGE_LABEL, MAIN_HANZI_FONT, dvalue(11));
			label->setName("ON_ONE_PAGE");
			pullShowLabel = Sprite::create();
			pullShowLabel->addChild(label);
		...
		else
		{
			...
			Label *label = Label::create(THE_NEXT_PAGE_LABEL, MAIN_HANZI_FONT, dvalue(11));
			label->setName("THE_NEXT_PAGE");
			pullShowLabel = Sprite::create();
			pullShowLabel->addChild(label);
			this->addChild(pullShowLabel);
		}

		Label *label = Label::create("正在加载", MAIN_HANZI_FONT, dvalue(11));
		label->setTag(105);
		waitingSprite = Sprite::create();
		waitingSprite->addChild(label);
		this->addChild(waitingSprite);
		waitingSprite->setVisible(false);

		Size size=Size(pullShowSprite->getContentSize().width+pullShowLabel->getContentSize().width, pullShowSprite->getContentSize().height);
	...
	void loosen()
	{
		if (m_isTop)
		{
			...
			auto label = pullShowLabel->getChildByName("ON_ONE_PAGE");
			((Label*)label)->setString(LOOSEN_ON_ONE_PAGE_LABEL);
		}
		else
		{
			pullShowSprite->setRotation(180);
			auto label = pullShowLabel->getChildByName("THE_NEXT_PAGE");
			((Label*)label)->setString(LOOSEN_THE_NEXT_PAGE_LABEL);
		}
	}
	...
	void pullWhack()
	{
		if (m_isTop)
		{
			pullShowSprite->setRotation(180);
			auto label = pullShowLabel->getChildByName("ON_ONE_PAGE");
			((Label*)label)->setString(ON_ONE_PAGE_LABEL);
		}
		else
		{
			pullShowSprite->setRotation(0);
			auto label = pullShowLabel->getChildByName("THE_NEXT_PAGE");
			((Label*)label)->setString(THE_NEXT_PAGE_LABEL);
		}
	}
	...
	void inWaiting()
	{
		...
		this->inWaitingActions(2);//这个是自行设置，可能有误

	}

	void inWaitingActions(float dt)
	{
		this->unschedule(schedule_selector(LPullShowSprite::inWaitingActions));

		auto label = waitingSprite->getChildByTag(105);
		string str=string(((Label*)label)->getString());

		if (strcmp(str.c_str(), "正在加载......") == 0)
		{
			((Label*)label)->setString("正在加载");//waitingSprite
		...
		else
		{
			...
			((Label*)label)->setString(str.c_str());//waitingSprite
	...
	void refurbishPoint()
	{
		Point pullShowSpritePoint=Point(pullShowSprite->getContentSize().width/2, this->getContentSize().height/2);

		pullShowSprite->setPosition(pullShowSpritePoint);

		Point pullShowLabelPoint=Point(pullShowSprite->getContentSize().width+pullShowLabel->getContentSize().width/2, this->getContentSize().height/2);

		pullShowLabel->setPosition(pullShowLabelPoint);

		Point waitingSpritePoint=Point(this->getContentSize().width/2, this->getContentSize().height/2);
...
void LyfTableView::registerTouch(){

	auto listener = EventListenerTouchOneByOne::create();
	listener->setSwallowTouches( true);
	listener->onTouchBegan = CC_CALLBACK_2(LyfTableView ::onTouchBegan, this );
	listener->onTouchMoved = CC_CALLBACK_2(LyfTableView ::onTouchMoved, this );
	listener->onTouchEnded = CC_CALLBACK_2(LyfTableView ::onTouchEnded, this );
	listener->onTouchCancelled = CC_CALLBACK_2 (LyfTableView ::onTouchCancelled, this );
...
void LyfTableView::unRegisterTouch(){
...
bool LyfTableView::initWithTableView(float contentSizeHeight){
	...
	if (!LayerColor::initWithColor(Color4B(255, 255, 255, 0)))//ccc4
	{
		return false;
	}
	this->setTouchMoved(true);
	this->setContentSize(Size(0, contentSizeHeight));

	m_backNode=Node::create();
	m_backNode->setPosition(Point(0, this->getContentSize().height));
...
{
	this->setContentSize(Size(_contentSize.width, contentSizeHeight));
	m_backNode->setPosition(Vec2(0, contentSizeHeight));//ccp
...
Rect LyfTableView::getRect(){

	return Rect(0,0,getContentSize().width,getContentSize().height);
}

bool LyfTableView::onTouchBegan(Touch *pTouch, Event *pEvent)
{
...
void LyfTableView::onTouchMoved(Touch *pTouch, Event *pEvent)
{
	float lenght=ccpDistance(pTouch->getLocationInView(), m_touchBeginUIPoint);

	Point point_last = pTouch->getPreviousLocation();
	point_last = this->convertToNodeSpace(point_last);

	Point point_this = pTouch->getLocation();
...
void LyfTableView::onTouchEnded(Touch *pTouch, Event *pEvent)
{
...
void LyfTableView::onTouchCancelled(Touch *pTouch, Event *pEvent)
{

	this->onTouchEnded(pTouch, pEvent);
}

void LyfTableView::inertia(float dt){
...
void LyfTableView::readDataInTurnLastPage(float dt){

	this->reLoadData();
	m_backNode->setPosition(Point(0, m_sizeHeight+this->getContentSize().height));
	CallFunc* callFunc=CallFunc::create(this, callfunc_selector(LyfTableView::PullShowHideFalse));
	Sequence* finish=Sequence::create(cellAction(getFirmFromCellPos(),0.3f), callFunc, NULL);
	m_backNode->runAction(finish);
}

void LyfTableView::readDataInTurnNextpage(float dt){

	this->reLoadData();
	m_backNode->setPosition(CCPointZero);
	CallFunc* callFunc=CallFunc::create(this, callfunc_selector(LyfTableView::PullShowHideFalse));
	Sequence* finish=Sequence::create(cellAction(getHomeFromCellPos(),0.3f), callFunc, NULL);
	m_backNode->runAction(finish);
}
...
void LyfTableView::cellGoHome(float dt){
...
void LyfTableView::setSelectedArray(CCArray *array){
...
void LyfTableView::setNoClickOtherArray(CCArray *array){
...
void LyfTableView::setIndicatorPointOff(float offX){
	...
	if (m_indicator)
	{
		m_indicator->setPointOff(Point(m_nIndicator_offX, 0));
	}
}
...
void LyfTableView::updatePullShow(){
	...
	Point pullUpSpritePoint=Point(this->getContentSize().width/2,
		-m_sizeHeight-pullUpShowSprite->getContentSize().height/2);
	Point pullDownSpritePoint=Point(this->getContentSize().width/2,
...
void LyfTableView::updateTableViewCellPos(){
	...
	{
		LyfTableViewCell* cell=m_TableViewCells.at(0);
	...
	for (int i=0; i<m_TableViewCells.size(); i++)
	{
		...
		cell->setAnchorPoint(Point(0.5f, 0.5f));

		cell->setPosition(Point(cell->getContentSize().width/2,
...
Point LyfTableView::getHomeFromCellPos()
{

	return Point( 0, this->getContentSize().height);
}

Point LyfTableView::getFirmFromCellPos()
{

	if (m_sizeHeight < this->getContentSize().height)
		return Point( 0, this->getContentSize().height);

	return Point( 0, m_sizeHeight);
}

ActionInterval* LyfTableView::cellAction(Point point, float time)
{

	MoveTo* moveto=MoveTo::create(time, point);
	EaseSineInOut* easeBack=EaseSineInOut::create(moveto);
	return easeBack;
}

void LyfTableView::visit(Renderer* renderer, const Mat4 &parentTransform, uint32_t parentFlags)
{

	glEnable(GL_SCISSOR_TEST);// 开启显示指定区域

	Point point = this->convertToWorldSpace(Point::ZERO);
	Size size = this->getContentSize();

	if (m_indicator);
	{
		float width=m_indicator->getPositionX() + m_indicator->getScaleX() * m_indicator->getContentSize().width/2;

		size.width = MAX(size.width, width);
	}

	CCEGLView* pGLView = Director::getInstance()->getOpenGLView();//CCEGLView::sharedOpenGLView();
	...
	Rect rect=Rect(point.x * glScaleX + off_X,
		point.y * glScaleY + off_Y,
		size.width * this->getScaleX() * glScaleX,
		size.height * this->getScaleY() * glScaleY);

	glScissor(rect.origin.x,rect.origin.y,rect.size.width,rect.size.height);// 只显示当前窗口的区域

	LayerColor::visit(renderer,parentTransform,parentFlags);// 调用下面的方法
