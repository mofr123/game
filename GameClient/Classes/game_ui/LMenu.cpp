...
,m_nTouchPriority((int)1)//kCCMenuHandlerPriority
{
    count1++;
    printf("LMenu:%d",count1);
}
...
void LMenu::onEnter(){
	...
	listener->onTouchCancelled = CC_CALLBACK_2(LMenu ::onTouchCancelled, this );
...
LMenu* LMenu::createWithRGBAProtocol(Node* backGround)
{
...
bool LMenu::initWithBackGround(Node *backGround){
...
bool LMenu::setBackGround(LMenuTouchState touchState, Node *var){
    ...
    if (Node* node=dynamic_cast<Node*>(var))
    {
        ...
        if (!m_preferredSize.equals(Size::ZERO) && !m_preferredSize.equals(node->getContentSize()))
        {
            if (Scale9Sprite* _var=dynamic_cast<Scale9Sprite*>(node))
            {
                _var->setPreferredSize(m_preferredSize);
            }
            if (LayerRGBA* _var=dynamic_cast<LayerRGBA*>(node))
            {
        ...
        if (touchState==LMenuStateHighlighted)
        {
			...
			m_bgHighlighted->setName("m_bgHighlighted");//add by 2014-11-15 9:12:00
        }
        ...
        if (touchState==LMenuStateDisabled)
        {
            ...
            m_bgDisabled=node;
			m_bgDisabled->setName("m_bgDisabled");//add by 2014-11-15 9:12:00
        }
...
void LMenu::updateWithPreferredSize(){
    ...
    if (m_bgNormal && !m_bgNormal->getContentSize().equals(m_preferredSize));
    {
        ...
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgNormal))
        {
    ...
    if (m_bgHighlighted && !m_bgHighlighted->getContentSize().equals(m_preferredSize))
    {
        ...
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgHighlighted))
        {
            node->setContentSize(m_preferredSize);
        }
    }
    ...
    if (m_bgDisabled && !m_bgDisabled->getContentSize().equals(m_preferredSize))
    {
        ...
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgDisabled))
        {
            node->setContentSize(m_preferredSize);
        }
    }
    ...
    if (m_bgSelected && !m_bgSelected->getContentSize().equals(m_preferredSize))
    {
        ...
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgSelected))
        {
            node->setContentSize(m_preferredSize);
        }
    }
...
void LMenu::updateWithPoint(){
    ...
    if (m_bgNormal)
    {
        if (NodeRGBA* node=dynamic_cast<NodeRGBA*>(m_bgNormal))
        {
    ...
    if (m_bgHighlighted)
    {
        if (NodeRGBA* node=dynamic_cast<NodeRGBA*>(m_bgHighlighted))
        {
            node->setPosition(point);
        }
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgHighlighted))
    ...
    if (m_bgDisabled)
    {
        if (NodeRGBA* node=dynamic_cast<NodeRGBA*>(m_bgDisabled))
        {
            node->setPosition(point);
        }
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgDisabled))
    ...
    if (m_bgSelected)
    {
        if (NodeRGBA* node=dynamic_cast<NodeRGBA*>(m_bgSelected))
        {
            node->setPosition(point);
        }
        if (LayerRGBA* node=dynamic_cast<LayerRGBA*>(m_bgSelected))
...
void LMenu::registerWithTouchDispatcher(){
    ...
    CCTouchDispatcher* touchDispatcher = Director::getInstance()->getTouchDispatcher();
...
void LMenu::setNormalColor(){
    ...
    if (getChildrenCount()>0)
    {
        ...
        {
            Node* node=(Node*)getChildren().at(i);
            
            //if (   strcmp(node->getName().c_str,m_bgHighlighted->getName().c_str)==0 //node->isEqual(m_bgHighlighted)
               // || strcmp(node->getName().c_str,m_bgDisabled->getName().c_str)==0//node->isEqual(m_bgDisabled)
               // || strcmp(node->getName().c_str,m_bgSelected->getName().c_str)==0)//node->isEqual(m_bgSelected)
           // {
               // break;
            //}
           ...
           // {
                ...
                node->setColor(Color3B(red,green,blue));//ccc3
...
void LMenu::setHighlightedColor(){
    ...
    if (getChildrenCount()>0)//this->getChildren()
    {
        for (int i=0; i<getChildrenCount(); i++)
        {
            Node* node=(Node*)getChildren().at(i);
...
void LMenu::setSprite(LMenuTouchState touchState, Node* var)
{
...
void LMenu::setSprite(LMenuTouchState touchState, Node* var, Point point){
...
Node* LMenu::getSprite(LMenuTouchState touchState){
...
void LMenu::setOpacity(GLubyte opacity){
    ...
    if (getChildrenCount()>0)
    {
        for (int i=0; i<getChildrenCount(); i++)
        {
           ...
           // if (Node* _children=dynamic_cast<Node*>(node))//CCRGBAProtocol
            //{
...
void LMenu::setColor(const Color3B &color3){
